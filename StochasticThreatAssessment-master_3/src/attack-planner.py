...
def generate_domain():
    ...
    print('types:' + ','.join(['host', 'vulnerability', 'file']), file=domain_string) 
    ...
    print('constants:' + ':'.join((hosts_string, files)), file=domain_string)
    ...
    print('predicates:' + ':'.join((p_1, p_2, p_3, p_4, p_5, 
                                               p_6, p_7, p_8, p_9, p_10, p_11)), file=domain_string)
    ...
    for profile in profile_list:
        print('\n'.join([action_exploit(vuln) for vuln in profile.vuln_list]), file=domain_string)
    print('\n'.join([action_access(), action_update_access()]), file=domain_string)
    ...
    # formatting
    types_ = types(*_types)
    constants_ = constants(*(getattr(types, k)(*v, sticky=1, subindentation_level=2) for k,v in list(_constants.items())),
...
def generate_problem_instance(host_gen, problem_file):
    ...
    print('init:' + ';'.join(p_init), file=problem_string)
    
    p_goal = 'goal:' + ';'.join(map(','.join,[('accessed', 'File')]))
    
    print(p_goal, file=problem_string)
...
if __name__ == '__main__':
    ...
    if TYPE == 'EVAL':
        ...
        with open(OUTPUT_FILE, 'a') as w:
            print(','.join([str(EVALUATION_TRIALS), str(successes), str(total_time / float(EVALUATION_TRIALS)), str(HOST_NUM), 
                                str(CONNECTEDNESS), str(NETWORK_ACCESS_PROB), str(ROOT_ACCESS_PROB), 
                                str(USER_ACCESS_PROB)]), file=w)
